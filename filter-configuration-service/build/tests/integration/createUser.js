...
test.serial("[POST /] user creation returns 201 and user object without password attribute", async (t) => {
    const res = await create(t.context.url, userA);
    t.is(res.status, 201);
    const user = await res.json();
    t.true(user.email === userA.email && user.password === undefined);
});
test.serial("[POST /] unique emil: should not create user with already existing email, returns 400", async (t) => {
